...
class XLMRClassificationTraining():
    """Finetunes XLMR model on a classification task"""
...
def main():
    parser = argparse.ArgumentParser(description="Train an XLMR model on a classification task.")
    ...
    parser.add_argument("--results_dir", help="Directory to save the trained model.", default="./results")
    ...
    #Encode labels
    label_dict = {'1. non-hateful': 0, '2. hateful': 1, '3. disagreement': 2}
    encoded_labels = np.array([label_dict[label] for label in labels])

    # Split the data into training and validation sets
    train_texts, val_texts, train_labels, val_labels = train_test_split(data, encoded_labels, test_size=.1)
    ...
    encoded_test_labels = np.array([label_dict[label] for label in test_labels])
    ...
    print(cm)
    print('Accuracy:' + str(accuracy_score(encoded_test_labels, predictions)))
    print('Micro F1 score:' + str(f1_score(encoded_test_labels, predictions, average='micro')))
    print('Macro F1 score:' + str(f1_score(encoded_test_labels, predictions, average='macro')))
    f1_scores = f1_score(encoded_test_labels, predictions, average=None)
    print("F1 non-hateful class: " + str(f1_scores[0]))
    print("F1 hateful class: " + str(f1_scores[1]))
    print("F1 disagreement class: " + str(f1_scores[2]))


    metrics = {
        'confusion_matrix': cm,
        'accuracy': accuracy_score(encoded_test_labels, predictions),
        'micro_f1_score': f1_score(encoded_test_labels, predictions, average='micro'),
        'macro_f1_score': f1_score(encoded_test_labels, predictions, average='macro'),
        'f1_scores': {
            'non-hateful': f1_scores[0],
            'hateful': f1_scores[1],
            'disagreement': f1_scores[2],
        },
    }

    # Save metrics to a pickle file in the specified directory
    metrics_file_path = os.path.join(args.results_dir, 'metrics_xlmr_disagreement_class.pkl')
...
# Example:
# python your_script_name.py --train_data path/to/train.csv --test_data path/to/test.csv --output_dir path/to/output --results_dir path/to/results --batch_size 32 --max_len 128 --epochs 3